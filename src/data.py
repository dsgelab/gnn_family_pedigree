...
class DataFetch():
    ...
    def __init__(self, maskfile, featfile, statfile, edgefile, params):
        ...
        self.static_data    = torch.tensor(stat_df[self.static_features].values, dtype=torch.float32)

        mask_df = pd.read_csv(maskfile)
        self.id_map                           = dict(zip(mask_df['node_id'], mask_df['FINREGISTRYID']))
    ...
    def construct_patient_graph(self, patient, all_relatives, all_x_static, all_y):
        """Creates a pytorch_geometric data object for the patient
        ...
        if self.params['use_edge'] == True:
            ...
            data = torch_geometric.data.Data(x=x_static, y=y)  
        ...
        # export graph image
        if (data.target_index%1000)==0:
            g = torch_geometric.utils.to_networkx(data)
            nx.draw(g)
            matplotlib.use("Agg") 
            fig.savefig("/data/projects/project_GNN/age_prediction/graph_plots/graph_patient_{}.png".format(data.target_index))
